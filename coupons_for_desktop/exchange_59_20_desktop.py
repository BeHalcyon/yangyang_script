...
def loopForDays(
    ...
    while True:
        ...
        next_task_start_time = (datetime.datetime.now() + datetime.timedelta(minutes=30)).replace(
            hour=(next_clock + 23) % 24,
            minute=59,
            second=5,
            microsecond=0)
        ...
        if 'DEBUG_59_20' in os.environ and os.environ['DEBUG_59_20'] == 'True':
            printT("\nTest end. Please set os.environ['DEBUG_59_20'] = \"False\"")
            break
...
if __name__ == '__main__':
    ...
    # Necessary
    # 服务器接口
    os.environ["JD_SIGN_API"] = ""
    os.environ["JD_SIGN_API_TOKEN"] = ""
    ...
    os.environ["JD_COOKIE"] = "&".join([
        'pin=jd_xxxxxxx;wskey=xxxxxxxxxxxxxxxxxxxx;whwswswws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;unionwsws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;',
        'pin=jd_xxxxxxx;wskey=xxxxxxxxxxxxxxxxxxxx;whwswswws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;unionwsws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;',
        'pin=jd_xxxxxxx;wskey=xxxxxxxxxxxxxxxxxxxx;whwswswws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;unionwsws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;',
        'pin=jd_xxxxxxx;wskey=xxxxxxxxxxxxxxxxxxxx;whwswswws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;unionwsws=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx;'
    ])
    ...
    loopForDays(
        second_ahead=0.3,                       # 线程启动的提前秒数；线程的冷冻时间；例如提前0.40s准备启动线程；依赖于设备资源
        sleep_time=0.03,                        # 每个线程的等待时间；每次post的等待时间；依赖于网络传输速率；少号时需适当调节
        thread_number=12,                       # 线程数量；post次数；可根据thread_number*sleep_time粗略计算当场运行时间；建议不超过15
        coupon_type="59-20",                    # 券名（可自定义），用于wxpusher通知
        clock_list=[0, 10, 14, 18, 21, 22])     # 定时任务；例如0点场、10点场...
