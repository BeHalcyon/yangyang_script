...
!(async() => {
    ...
    if(process.env.JD_TRY && process.env.JD_TRY === 'true'){
        ...
        console.log("我只想运行前8个账号，其他账号还是算了吧！")
        for(let i = 0; i < $.cookiesArr.length && i < 8; i++){
            if($.cookiesArr[i]){
                $.cookie = $.cookiesArr[i];
                ...
                if($.isForbidden === false && $.isLimit === false){
                    ...
                    for(let i = 0; i < trialActivityIdList.length && $.isLimit === false; i++){
                        ...
                        await try_apply(trialActivityTitleList[i], trialActivityIdList[i], trialActivityItemList[i])
                            ...
                            if(item.skuTitle && $.isPush){
                                ...
                                if(args_xh.whiteList){
                                    if(args_xh.whiteListKeywords.some(fileter_word => item.skuTitle.includes(fileter_word))){
                                        ...
                                        trialActivityItemList.push(item)
                                    }
                                } else {
                                    tempKeyword = ``;
...
function try_apply(title, activityId, activityItem){
    return new Promise((resolve, reject) => {
            ...
            try{
                if(err){
                    ...
                    if(data.success && data.code === "1"){  // 申请成功
                        // console.log("申请提交成功")
                        // 打印申请信息
                        let p = 1000*parseFloat(activityItem.supplyNum)/parseFloat(activityItem.applyNum)
                        console.log(`🎉已申请：${activityItem.skuTitle.substr(0, 15)}🎉价格：${parseFloat(activityItem.jdPrice)}🎉命中率：${p.toFixed(2)}‰|||`)
                        $.totalSuccess++
                        ...
                        console.log(data.message)   // 当前活动不能申请
