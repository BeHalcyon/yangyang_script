...
def exchangePayCoupon(header='https://api.m.jd.com/client.action?functionId=newBabelAwardCollection&screen=800*1324&client=wh5&clientVersion=1.0.0',
    ...
    # 存入数据库
    database = SQLProcess(json.dumps(body['actKey']).replace('=', '').replace(',', '').replace('{', '').replace('}', '').replace('"', '').replace(' ', '').replace(':', ''), database_dict)
    # 插入所有数据，如果存在则更新
    insert_start = time.time()
    for i, ck in enumerate(cookies):
        database.insertItem(ck, time.time(), str(datetime.date.today()), len(cookies) - i)
    ...
    for process_id in range(thread_number):
        ck = cookies[process_id % len(cookies)]
        log_dict = json.loads(log_str_arr[process_id])
        body['log'] = log_dict['log']
        body['random'] = log_dict['random']
